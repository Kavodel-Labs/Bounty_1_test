-- ===================================================
-- DAILY EMAIL REPORT - V4 (CASH GGR IMPLEMENTATION)
-- ===================================================
-- Purpose: Implement Cash GGR approach
-- Cash GGR = Cash Bets - Cash Wins (withdrawable balance only)
-- Hold% = Cash GGR / Cash Turnover × 100
-- NGR = Cash GGR - Bonus Costs (recognized freebet cost)
-- Updated: September 2025
-- NOTE: This is the ORIGINAL version provided by user
-- See daily_email_report.sql for CTO-APPROVED adjusted version
...
final_calculations AS (
  SELECT
    *,
    ...
    -- NGR CALCULATIONS (Cash GGR - Bonus Cost)
    -- ⚠️ WARNING: MISSING FEES - See adjusted version for CTO-approved calculation
    (cash_ggr_yesterday - bonus_cost_yesterday) as ngr_yesterday,
    (cash_ggr_mtd - bonus_cost_mtd) as ngr_mtd,
    (cash_ggr_prev_month - bonus_cost_prev_month) as ngr_prev_month
...
ngr_estimations AS (
  SELECT
    *,
    (cash_ggr_estimation - bonus_cost_estimation) as ngr_estimation
