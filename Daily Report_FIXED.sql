/* =========================
   ...
   [FIXED] Currency conversion now standardized with Monthly/LTV reports
   [FIXED] Deposits now use proper fallback: COALESCE(eur_amount, amount)
   [FIXED] All metrics use 4-level currency hierarchy
   [UPDATED] Added Bonus Cost Ratio and Turnover Factor columns
   ========================= */
...
player_reg AS (
  ...
  WHERE 1=1
    -- Only filter by currency if non-EUR is selected (EUR uses eur_amount fallback, so no currency filter needed)
    -- Filter by player_balances currency_type when non-EUR currency is selected
...
ftd_metrics AS (
  SELECT
    ds.report_date,
    -- #FTDs: Distinct count of users that have their first successful deposit in the timeframe
    COUNT(DISTINCT f.player_id) AS ftds_count,
    -- #New FTDs: FTDs that registered in the same month as FTD month (excluding D0 FTDs)
    COUNT(*) FILTER (WHERE DATE_TRUNC('month', f.registration_ts) = DATE_TRUNC('month', f.first_deposit_ts)
                      AND DATE_TRUNC('day', f.registration_ts) <> DATE_TRUNC('day', f.first_deposit_ts)) AS new_ftds,
    -- #Old FTDs: FTDs that registered previous to FTD month
    COUNT(*) FILTER (WHERE DATE_TRUNC('month', f.registration_ts) < DATE_TRUNC('month', f.first_deposit_ts)) AS old_ftds,
    -- D0 FTDs: FTDs with the registration in the same day
    COUNT(*) FILTER (WHERE DATE_TRUNC('day', f.registration_ts) = DATE_TRUNC('day', f.first_deposit_ts)) AS d0_ftds,
    -- Late FTDs: FTDs with the registration not in the same day
...
/* ---------- DEPOSITS (FIXED - NOW CONSISTENT WITH MONTHLY/LTV) ---------- */
deposit_metrics AS (
  SELECT
    ds.report_date,
    ...
    COALESCE(SUM(CASE
      ...
      THEN 
        CASE 
          -- Native currency: use raw amount
          WHEN t.currency_type = {{currency_filter}}
          THEN t.amount
          -- Converting to EUR: use eur_amount with fallback to amount
          WHEN {{currency_filter}} = 'EUR'
          THEN COALESCE(t.eur_amount, t.amount)  -- ✅ FIXED
          ELSE t.amount
        END
...
/* ---------- WITHDRAWALS (FIXED - NOW CONSISTENT WITH MONTHLY/LTV) ---------- */
  ...
  SELECT
    ...
    COALESCE(SUM(ABS(
      CASE 
        ...
        WHEN {{currency_filter}} = 'EUR'
        THEN COALESCE(t.eur_amount, t.amount)  -- ✅ Already correct
        ELSE t.amount
      END
    )) FILTER (
      WHERE t.transaction_category='withdrawal'
...
/* ---------- BETTING METRICS (FIXED - NOW CONSISTENT) ---------- */
betting_metrics AS (
  SELECT
    ds.report_date,
    COALESCE(SUM(CASE
...
/* ---------- BONUS METRICS (FIXED - NOW CONSISTENT) ---------- */
bonus_converted AS (
  SELECT
    ds.report_date,
    COALESCE(SUM(CASE
...
/* ---------- GRANTED BONUS (FIXED - NOW CONSISTENT) ---------- */
granted_bonus AS (
  SELECT
    ds.report_date,
    COALESCE(SUM(CASE
...
daily_data AS (
  SELECT 
    ...
    -- NEW: Bonus Cost Ratio = Bonus Cost / Granted Bonus
    ...
    -- NEW: Turnover Factor = Total Turnover / Total Deposit
    ROUND(CASE WHEN COALESCE(dm.deposits_amount,0) > 0 
               THEN (COALESCE(bet.cash_bet,0) + COALESCE(bet.promo_bet,0)) / dm.deposits_amount ELSE 0 END, 2) AS "Turnover Factor"
  FROM date_series ds
